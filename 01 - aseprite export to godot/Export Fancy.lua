            ...
            if app.pixelColor.rgbaA(sourcePixel) > 0 then
                ...
                for colorIndexCurrent, colorLayer in ipairs(colorLayers) do
                    ...
                    if colorCel ~= nil then
                        local colorPixel = celPixel(colorCel, x, y)
                ...
                if colorIndex >= 0 then
                    ...
                    local replacementColor = Color { r = colorValue, g = colorValue, b = colorValue, a = 255 }
        ...
        for frameIndex, frame in ipairs(sprite.frames) do
            ...
            for layer in values(rootGroup.layers) do
                ...
                if layer.isVisible and not stringStarts(layer.name, "-") and mainCel ~= nil then
                    drawMain(mainCel, frameImage, colorLayers, frameIndex)
                ...
                for x = 0, source.width - 1 do
                    for y = 0, source.height - 1 do
                        ...
                        if app.pixelColor.rgbaA(pixel) > 0 then
                            groupFile:write("anchorPosition=" .. (x + cel.position.x) .. "," .. (y + cel.position.y) .. "\n")
                        end
                    end
                end
            end
...
app.alert {
    title = "Fancy Export",
    text = "groups [" .. exportedGroupCount .. "] cels [" .. exportedCelCount .. "] grid [" .. exportedImageGrid .. "x" .. exportedImageGrid .. "]",
    buttons = { "OK" }
}